              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    const Text('Smart Valve Management', style: TextStyle(fontSize: 27, fontWeight: FontWeight.bold)),
                    const Text('Monitor  •  Control  •  Manage'),
                    const SizedBox(height: 24),
                    const Text('Login', style: TextStyle(fontSize: 30, fontWeight: FontWeight.bold)),
                    const Text('Agent / Operator'),
...
class _HomeState extends State<Home> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Smart Valve Management')),
        ...
        child: Card(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                const Text('Select Zone and Ward', textAlign: TextAlign.center, style: TextStyle(fontSize: 26, fontWeight: FontWeight.bold)),
                const SizedBox(height: 18),
                DropdownButtonFormField<int>(
                  value: zone,
                  decoration: const InputDecoration(labelText: 'Zone No', border: OutlineInputBorder()),
                  items: [1, 2, 3, 4, 5].map((x) => DropdownMenuItem(value: x, child: Text('Zone $x'))).toList(),
                  onChanged: (x) => setState(() => zone = x),
                ),
                const SizedBox(height: 7),
                OutlinedButton.icon(onPressed: () => add('Zone'), icon: const Icon(Icons.add), label: const Text('ADD ZONE')),
                const SizedBox(height: 12),
                DropdownButtonFormField<int>(
                  value: ward,
                  decoration: const InputDecoration(labelText: 'Ward No', border: OutlineInputBorder()),
                  items: [1, 2, 3, 4, 5, 6, 7, 8].map((x) => DropdownMenuItem(value: x, child: Text('Ward $x'))).toList(),
                  onChanged: (x) => setState(() => ward = x),
                ),
                const SizedBox(height: 7),
                OutlinedButton.icon(onPressed: () => add('Ward'), icon: const Icon(Icons.add), label: const Text('ADD WARD')),
...
class Shell extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Scaffold(body: child, bottomNavigationBar: Nav(index: index, onTap: (i) {
      if (i == 0) {
        ...
        child: Column(
          children: [
            ...
            Card(
              child: Column(
                children: [
                  const ListTile(title: Text('Valve Position', style: TextStyle(fontWeight: FontWeight.bold)), subtitle: Text('Requested / Actual')),
                  Slider(value: pos, divisions: 100, label: '${pos.round()}%', onChanged: (x) => setState(() => pos = x)),
                  FilledButton(onPressed: () {}, child: Text('SET VALVE TO ${pos.round()}%')),
                  const SizedBox(height: 8),
                  Row(
                    children: [
      ...
      body: Column(
        children: [
          Padding(
            ...
            child: Row(
              children: [
                const Expanded(child: Text('Municipal Valve Map', style: TextStyle(fontSize: 19, fontWeight: FontWeight.bold))),
          ...
          Expanded(
            child: FlutterMap(
              ...
              children: [
                TileLayer(urlTemplate: 'https://tile.openstreetmap.org/{z}/{x}/{y}.png', userAgentPackageName: 'com.municipal.operator'),
          ...
          const Padding(
            ...
            child: Align(alignment: Alignment.centerLeft, child: Text('Valve List', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold))),
          ),
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, i) => ListTile(
                ...
                title: Text('Valve ID: ${valves[i]}'),
                subtitle: const Text('Latitude / Longitude'),
                trailing: IconButton(onPressed: () {}, icon: const Icon(Icons.link)),
              ),
            ),
          ),
        ],
